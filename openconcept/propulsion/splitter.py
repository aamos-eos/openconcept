...
class PowerSplit(ExplicitComponent):
    ...
    def compute(self, inputs, outputs):
        ...
        if rule == "fraction":
            ...
            over_supply_idx = np.where(inputs["power_in"] < inputs["power_split_amount"])
            po_A = np.zeros(nn)
            po_B = np.zeros(nn)
            po_A[over_supply_idx] = inputs["power_in"][over_supply_idx] * eta
            po_B[over_supply_idx] = np.zeros(nn)[over_supply_idx]
            # else:
            partial_supply_idx = np.where(inputs["power_in"] >= inputs["power_split_amount"])
            po_A[partial_supply_idx] = inputs["power_split_amount"][partial_supply_idx] * eta
            po_B[partial_supply_idx] = (inputs["power_in"][partial_supply_idx] - inputs["power_split_amount"][partial_supply_idx]) * eta
