...
class ComputeMotorPower(om.ExplicitComponent):
    ...
    def setup(self):
        ...
        self.declare_partials('mech_power', ['power_lim'], rows=ar, cols=ar)
        self.declare_partials('torque', ['torque_cmd', 'rpm'], rows=ar, cols=ar)
        self.declare_partials('torque', ['power_lim'], rows=ar, cols=ar)
    ...
    def compute_partials(self, inputs, partials):
        torque_cmd = inputs['torque_cmd']
        rpm = inputs['rpm']
        power_lim = inputs['power_lim'] * 1000  # Convert kW to W
        ...
        power_limited = mech_power_req > power_lim
        ...
        partials['mech_power', 'power_lim'] = np.where(power_limited, 1000, 0)  # 1000 for kW to W conversion
        ...
        partials['torque', 'rpm'] = np.where(power_limited, 
                                                   -power_lim / (omega_safe**2) * 2 * np.pi / 60, 
                                                   0)
        partials['torque', 'power_lim'] = np.where(power_limited, 1000 / omega_safe, 0)
...
class EmpiricalMotor(om.Group):
    ...
    def setup(self):
        ...
        compute_elec = ComputeMotorElecDraw(num_nodes=num_nodes)
        
        # Add all subsystems
        self.add_subsystem('motor_voltage_interp', motor_voltage_interp, promotes_inputs=['rpm', 'voltage'], promotes_outputs=['mech_power_lim'])
        self.add_subsystem('compute_power', compute_power, promotes_inputs=['torque_cmd', 'rpm', 'voltage', 'mech_power_lim'], promotes_outputs=['mech_power', 'torque_actual'])
        self.add_subsystem('motor_eff_interp', motor_eff_interp, promotes_inputs=['rpm'], promotes_outputs=['eff'])
        self.add_subsystem('compute_elec', compute_elec, promotes_inputs=['mech_power', 'eff'], promotes_outputs=['elec_power'])
        
        # Connect torque_actual to motor_eff_interp
        self.connect('compute_power.torque_actual', 'motor_eff_interp.torque')
...
def test_motor_components():
    ...
    torque_actual = prob.get_val('torque_actual', units='N*m')
    ...
    print(f"Actual torque: {torque_actual} N*m")
